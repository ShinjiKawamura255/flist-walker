...
| Step 4 Root 切替 | partial automated | `shortcuts.rs`, `search_filelist.rs`, `session_tabs.rs` | root change cleanup と root selector interaction を owner test へ追加 |
| Step 5 Tab 操作 | partial automated | `session_tabs.rs`, `render_tests.rs`, `shortcuts.rs` | tab switch/close/reorder の代表 flow を owner test へ追加 |
| Step 6 FileList 作成 | partial automated | `index_pipeline/dialogs_and_inflight.rs`, `index_pipeline/search_filelist.rs`, `index_pipeline/filelist_lifecycle.rs`, `render_tests.rs` | dialog / background walker 準備 flow を優先追加 |
