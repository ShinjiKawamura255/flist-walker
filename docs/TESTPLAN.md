...
- `rust/src/app/tests/session_tabs.rs` の `tab_state_contract_round_trip_pins_field_layout` は `TabIndexState` / `TabQueryState` / `TabResultState` / `AppTabState` の field layout を固定する contract regression として扱う。
...
| TC-011 | manual | GUI 回帰手順に基づく再検証が可能（`scripts/gui-smoke-fixture.sh` と `docs/GUI-TESTREPORT.md` を使用） | SP-011 |
...
| TC-124 | unit+perf | adaptive walker は既定 backend として jwalk と同一 dataset の候補件数を一致させつつ elapsed time と read_dir 制御指標を診断出力できる。developer-only `walker_backend = "jwalk"` で jwalk へ戻せる。Windows の Hidden+System+ReparsePoint 互換 junction は候補化せず、低コア環境向けの single-worker serial fast path は同じ候補を返して metrics を記録する | SP-002, SP-007, SP-016 |
...
| TC-117 | unit | `FLISTWALKER_DISABLE_SELF_UPDATE=1` では起動時更新確認を送らず、更新チェックも `None` を返して自己更新を無効化する | SP-014 |
...
- TC-117 -> SP-014 -> DES-014 -> FR-019, FR-020
