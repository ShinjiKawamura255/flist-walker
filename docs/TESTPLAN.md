...
- Perf: 軽量 PR gate は `perf_regression_filelist_stream_matches_v0123_reference_budget` とし、FileList stream budget を現行 control baseline に載せる。heavy suite は `perf_walker_classification_is_faster_than_eager_metadata_resolution` として分離する。
...
| TC-082 | unit+perf | 回帰: FileList の `\` 区切り候補を filesystem existence probe なしでプラットフォーム優先解釈し、現行 control baseline から 5% 以内の変動に維持する | SP-001, SP-007 |
...
- lightweight PR perf gate: `.github/workflows/ci-cross-platform.yml` の linux-native job で `perf_regression_filelist_stream_matches_v0123_reference_budget` を実行し、現行 control baseline を監視する
...
- 期待動作: `\` / `/` の差異は候補順序だけで吸収し、初期ストリームは現行 control baseline 相当の line-only 解析速度を維持する。
