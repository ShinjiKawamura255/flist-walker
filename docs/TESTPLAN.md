...
- runtime settings は `~/.flistwalker_config.json` に集約し、`FLISTWALKER_*` は初回 seed としてのみ扱う。環境変数は validation 上 `dev/test override`、`build/release` に分けて扱う。`README.md` では config file の場所と seed-only 挙動を明記し、dev/test override は `TESTPLAN.md` と実装近傍 test に閉じる。
- GUI Manual:
...
| TC-110 | unit | ignore list ファイルの候補は既定有効で除外され、GUI の `Ignore List` チェックボックスで有効/無効を切り替えられる | SP-015 |
