...
- 役割補足: `frame update cycle` では `app/mod.rs` が egui frame ごとの orchestration と repaint 判断だけを持ち、`app/pipeline.rs` は index response の frame budget と route effect dispatch、`app/index_response_arbitration.rs` は tab/UI state を変更しない Active-first / Warm-second / request-id 順の mailbox 選択と quota、`app/index_coordinator.rs` は generation/role/mailbox tracking を担当する。background tab mutation は `app/tabs.rs`、finalization の identity/policy/input payload と増分 workspace/advance は `app/index_finalization.rs`、active result refresh は `app/pipeline_owner.rs`、render command 生成は `app/render.rs` が担当する。
