...
- 役割補足: `app/tabs.rs` は tab 初期化、tab snapshot capture/apply、tab switch/move/close、新規 tab 作成に加え、action/sort request routing の owner API と background tab 向け search/index response consume helper、activation 時の refresh 入口を担当する。live 側の tab/session registry（`tabs`, `active_tab`, `next_tab_id`, `pending_activation_refresh`, request routing）は `app/state.rs` の `TabSessionState` に束ねる。`pending_activation_refresh` は session/closed-tab restore だけでなく、preempt または app pending queue eviction で replacement を失った tab の再 activation refresh も所有する。`ClosedTabState` は古い request ownership ではなく、失われた index/search/sort work の再要求 marker と preview reload marker を新しい tab id へ引き継ぐ。`TabSessionState` は Vec そのものを公開せず、明示的な collection API と active tab / tab id / routing / activation-refresh helpers を通じてのみ更新する。
