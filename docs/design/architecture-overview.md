...
- 役割補足: preset picker は OS primary modifier の `Primary+Shift+P` から開く modal とし、catalog を専用 worker で非同期 load して名前を fuzzy filter する。Enter は pure search state だけを現在 tab へ適用し、F2は選択中presetのdraft editorへ遷移する。picker/editorからNamed Root managerへ遷移し、追加・rename/repath・確認付きdeleteも同じworker経由でatomic更新する。renameは参照presetを追従させ、deleteはsnapshot fallbackへ移行する。catalog編集は背景actionや現在tabを変更せず、メインpanelへpreset controlは常設しない。
