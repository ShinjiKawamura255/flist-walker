...
- 役割補足: query history の検索順位・exact duplicate 最新化・100件上限は `query_history.rs`、Unicode character index と Emacs 風編集 reducer は `text_editing.rs` に置き、GUI/TUI adapter は event/selection の変換だけを担当する。
...
- 役割補足: preset picker はメインpanelの`Presets...` top actionとOS primary modifierの`Primary+Shift+P`から同じopen transitionへ入るmodalとし、catalog を専用 worker で非同期 load して名前を fuzzy filter する。Enter は pure search state だけを現在 tab へ適用し、Addは現在tabから新規draft、F2は選択中presetの編集draft、Deleteは確認状態へ遷移する。presetとNamed Rootのadd/replace/deleteは同じworker経由でatomic更新する。Named Root renameは参照presetを追従させ、deleteはsnapshot fallbackへ移行する。catalog編集は背景actionや現在tabを変更せず、メインpanelにはpicker launcherだけを置いて一覧・選択・編集UIを埋め込まない。
- 役割補足: GUI の `次へ` / `前へ` / `確定` / `キャンセル` は `app/input/shortcuts.rs` の共有 semantic helper が通常キーと、設定有効時だけの Emacs chord を同じ command へ変換する。結果一覧、履歴、root dropdown、preset/Named Root、確認・通知 modal はこの境界を使い、新しい対話面で `ArrowDown` / `ArrowUp` / `Enter` / `Escape` と Emacs 判定を feature ごとに複製しない。
...
- 役割補足: `AppRuntimeState` の committed result payload は private aggregate とし、read-only projection だけを `Deref` で公開する。更新は entry snapshot、base/result ranking、match count、selection row、preview の目的別 API、tab/reclaimer 境界は aggregate の swap/take/restore API を通し、production code に whole-payload mutable reference を公開しない。
- 役割補足: tab activation は `TabSessionState` から slot を一時的に remove して owned value とし、outgoing live payload を slot へ store、active index 更新、incoming root/filter の適用と payload の shell への load、scratch slot の reinsert、post-activation の順で行う。`&AppTabState` を保持したまま `&mut FlistWalkerApp` を更新せず、index/pending/kind/incremental/result container と entry-kind cache は move/swap し、activation hot path では allocator compaction、全件 drop、result rebuild を行わない。inactive tab の計算済み Results は保持し、Preview 本文だけを reload pending 付きで解放する。
...
- 役割補足: `frame update cycle` では `app/mod.rs` が egui frame ごとの orchestration と repaint 判断だけを持ち、`app/pipeline.rs` は index response の frame budget、route 選択、typed effect dispatch、`app/index_response_arbitration.rs` は tab/UI state を変更しない Active-first / Warm-second / request-id 順の mailbox 選択と quota、`app/index_coordinator.rs` は generation/role/mailbox tracking を担当する。`app/index_response_effects.rs` は active/stale response、terminal ownership、reclaim、incremental completion を適用して loop/frame effect を返し、background payload mutation は同 owner から `app/tabs.rs` の reducer へ委譲する。finalization の identity/policy/input payload と増分 workspace/advance は `app/index_finalization.rs`、active result refresh は `app/pipeline_owner.rs`、render command 生成は `app/render.rs` が担当する。
- 役割補足: `shutdown/persist` では `app/mod.rs` が eframe callback から shutdown seam を呼ぶだけに留め、UI state 永続化は `app/session.rs`、worker stop/join は `app/worker/runtime.rs` が担当する。
...
- 役割補足: `trace helper` では `app/mod.rs` が opt-in window trace の入口だけを保持し、worker trace は `app/worker/tasks.rs` の実装 body と `app/worker/protocol.rs`、session/window 永続化まわりの補助 trace は `app/session.rs` など各 owner helper へ分離する。
...
- 役割補足: `app/state.rs` は filelist/update dialog 状態、sort metadata、entry kind、tab drag など GUI 横断で共有される state 型を集約し、index finalization の高容量 workspace は `app/index_finalization.rs` に分離して `FlistWalkerApp` 本体から型定義のノイズを外す。
...
- 役割補足: worker request/response channel は `app/worker/bus.rs` へ集約し、`FlistWalkerApp` 直下には worker bus 全体を 1 フィールドで保持する。
...
- 役割補足: index coordinator は dispatch 済み要求を最大 2 件、app pending scheduler は全体最大 4 件かつ tab ごとに最新 1 件だけ所有する。dispatch 済み index request は replacement によって stale になっても terminal response まで tracking に残し、worker channel の送信成功時だけ inflight に加える。これにより coordinator の空き判定と物理 worker FIFO を一致させ、active-tab request が未追跡の stale backlog 後方へ入ることを防ぐ。kind owner は `Full` で取り出した path を queue 先頭へ戻し、worker からの `kind=None` terminal response で pending/in-flight set を解放する。
...
- 役割補足: `app/index_worker.rs` は FileList / Walker streaming、kind classification、index worker 実装を担当し、その他の worker spawn/use-case は `app/worker/tasks.rs` に集約する。
- 役割補足: `app/worker/protocol.rs` は search/index/preview/action/sort/kind/filelist/update/root-validation の request/response 型を集約し、worker protocol surface を worker 実装や bus wiring から独立して保守できるようにする。保存済み root の filesystem validation は Catalog worker と pending identity を共有せず、専用 `RootValidationWorkerBus` が request_id、dialog generation、intent/index、draft snapshot を受けて canonical duplicate 判定まで所有する。
- 役割補足: `app/worker/runtime.rs` は worker shutdown signal と join timeout の管理だけを持ち、個別 worker 実装から runtime orchestration を分離する。
- 役割補足: `app/worker/runtime.rs` は action 2、kind 1、index 2 の固定 worker handle を名前付きで直接所有し、stop accepting -> queued request settle -> join -> response close を全体 250ms の終了予算で調停する。
...
- 役割補足: action authorization policy は `actions.rs`、worker の共有 action notice formatting は `app/worker/tasks.rs` が直接所有し、re-export 専用の中間 module を置かない。
...
- 役割補足: runtime settings は Windows では `%LocalAppData%\flistwalker\`、Linux/macOS では `~/.flistwalker/` へ集約し、`FLISTWALKER_*` は初回 seed としてのみ使う。初回 seed には一般利用者向けの `walker_max_entries`、`history_persist_disabled`、`restore_tabs_enabled`、`emacs_keybindings_enabled`、`ctrl_w_deletes_word_in_query`、`tab_pin_moves_to_next_row` を既定値で含め、既存 config に欠けている場合も読み込み時に同じ項目を補完する。build/release と dev/test override は従来どおり env のまま保持し、公開 docs には config file の場所と seed-only 挙動を明記する。
- 役割補足: `walker_runtime/` は GUI/TUI 共通の adaptive traversal、runtime limit 解決、entry classification、truncation notice を UI 非依存で提供する。`app/index_worker.rs` は GUI protocol/batching/tracing/metrics を所有し、`cli_tui/` は path-only batch と request identity 付き truncation を所有する。FileList 作成の完全 snapshot は別経路に保つ。
- 役割補足: candidate は `entry.rs` の `Entry { path, kind }` で app/index/search worker 境界をまたいで表現し、app 側の kind side-channel を持たない。
- 実装: `rust/src/app/mod.rs`, `rust/src/app/coordinator.rs`, `rust/src/app/filelist/mod.rs`, `rust/src/app/update.rs`, `rust/src/app/render.rs`, `rust/src/app/input/mod.rs`, `rust/src/app/session.rs`, `rust/src/app/state.rs`, `rust/src/app/tab_state.rs`, `rust/src/app/tabs.rs`, `rust/src/app/pipeline.rs`, `rust/src/app/pipeline_owner.rs`, `rust/src/app/bootstrap.rs`, `rust/src/app/cache.rs`, `rust/src/app/result_reducer.rs`, `rust/src/app/result_flow.rs`, `rust/src/app/preview_flow.rs`, `rust/src/app/worker/bus.rs`, `rust/src/app/worker/protocol.rs`, `rust/src/app/worker/runtime.rs`, `rust/src/app/shell_support.rs`, `rust/src/app/ui_state.rs`, `rust/src/app/query_state.rs`, `rust/src/app/search_coordinator.rs`, `rust/src/app/index_coordinator.rs`, `rust/src/app/index_worker.rs`, `rust/src/app/worker/tasks.rs`, `rust/src/entry.rs`, `rust/src/ui_model/mod.rs`, `rust/src/query.rs`, `rust/src/query_history.rs`, `rust/src/text_editing.rs`, `rust/src/search/mod.rs`, `rust/src/search/cache.rs`, `rust/src/search/config.rs`, `rust/src/search/execute.rs`, `rust/src/search/rank.rs`, `rust/src/ignore_list.rs`, `rust/src/indexer/mod.rs`, `rust/src/indexer/filelist_reader.rs`, `rust/src/indexer/walker.rs`, `rust/src/indexer/filelist_writer.rs`
- 実装: `rust/src/app/mod.rs`, `rust/src/app/coordinator.rs`, `rust/src/app/filelist/mod.rs`, `rust/src/app/update.rs`, `rust/src/app/render.rs`, `rust/src/app/input/mod.rs`, `rust/src/app/session.rs`, `rust/src/app/state.rs`, `rust/src/app/tab_state.rs`, `rust/src/app/tabs.rs`, `rust/src/app/pipeline.rs`, `rust/src/app/pipeline_owner.rs`, `rust/src/app/bootstrap.rs`, `rust/src/app/cache.rs`, `rust/src/app/result_reducer.rs`, `rust/src/app/result_flow.rs`, `rust/src/app/preview_flow.rs`, `rust/src/app/worker/bus.rs`, `rust/src/app/worker/protocol.rs`, `rust/src/app/worker/runtime.rs`, `rust/src/app/shell_support.rs`, `rust/src/app/ui_state.rs`, `rust/src/app/query_state.rs`, `rust/src/app/search_coordinator.rs`, `rust/src/app/index_coordinator.rs`, `rust/src/app/index_worker.rs`, `rust/src/app/worker/tasks.rs`, `rust/src/entry.rs`, `rust/src/ui_model/mod.rs`, `rust/src/query.rs`, `rust/src/query_history.rs`, `rust/src/text_editing.rs`, `rust/src/runtime_config.rs`, `rust/src/search/mod.rs`, `rust/src/search/cache.rs`, `rust/src/search/config.rs`, `rust/src/search/execute.rs`, `rust/src/search/rank.rs`, `rust/src/ignore_list.rs`, `rust/src/indexer/mod.rs`, `rust/src/indexer/filelist_reader.rs`, `rust/src/indexer/walker.rs`, `rust/src/indexer/filelist_writer.rs`
...
- 実装: `.github/workflows/ci-cross-platform.yml`, `.github/workflows/ci-policy-guardian.yml`, `.github/workflows/security-audit.yml`, `.github/workflows/ci-canary.yml`, `.github/workflows/dependabot-auto-merge.yml`, `.github/workflows/perf-regression.yml`, `.github/workflows/release-tagged.yml`, `.github/dependabot.yml`, `scripts/check_ci_policy.py`, `scripts/tests/test_check_ci_policy.py`, `scripts/prepare-release.ps1`, `scripts/test-prepare-release-archive.ps1`, `rust/rust-toolchain.toml`, `rust/.cargo/audit.toml`, `docs/CI_OPERATIONS.md`, `docs/OSS_COMPLIANCE.md`
...
- 実装: `rust/src/updater.rs`, `rust/src/updater/release.rs`, `rust/src/updater/staging.rs`, `rust/src/updater/manifest.rs`, `rust/src/updater/apply.rs`, `rust/src/updater/transaction.rs`, `rust/src/updater/transaction/model.rs`, `rust/src/updater/transaction/filesystem.rs`, `rust/src/updater/transaction/platform.rs`, `rust/src/main.rs`, `rust/src/app/update.rs`, `rust/src/app/render.rs`, `rust/src/app/worker/tasks.rs`, `rust/src/app/state.rs`
...
- 役割補足: activation は executable parent 内の same-directory preparation、create-new lock、per-target write-ahead marker、parent/helper registration acknowledgement、sidecars-first/binary-last commit、platform atomic replacement、backup rollback、startup recovery を 1 transaction として管理する。Windows の既存 target は外部 shell を起動せず updater process 内の native `ReplaceFileW` で置換し、ambiguous state は証跡を保持して停止する。
- 役割補足: candidate は release metadata/support と optional `AutoUpdateAssets` を分離し、OS/CPU support を asset lookup より先に決める。manual-only candidate は install payload を持たず、apply 入口は `Auto + Some(payload)` の invariant を再検証する。
- 役割補足: GUI と update worker は per-request atomic control を共有し、cancelable staging と transaction/helper handoff の境界を CAS で固定する。native/signal close は staging を cancel して `CancelClose` で待機し、handoff 開始後は acknowledgement まで cancellation を観測しない。
- 役割補足: helper failure record は acknowledgement から得た検証済み install directory だけへ bounded structured record として best-effort 保存する。rollback 後の旧 binary 再起動では spawn 前に publish して startup read との race を避け、GUI startup は recovery と helper の両診断を保持したまま suppressible check error と別の one-shot modal へ渡す。
- 役割補足: transaction coordinator は状態遷移を調停し、marker-v1 schema は `model`、path/type/hash の再検証と artifact policy は `filesystem`、process 待機・再起動と OS native replacement は `platform` が所有する。platform から model/filesystem への逆依存を禁止し、serialized marker contract と rollback/recovery policy を構造変更から隔離する。
...
- 役割補足: `render_dialogs/update.rs` は update dialog の描画と入力取得を担当し、`render.rs` は `RenderUpdateDialogCommand` の queue/dispatch 境界を保持する。永続化 (`session.rs`) と app close orchestration (`app/mod.rs`) は `FlistWalkerApp` 側に残したまま `UpdateAppCommand` で橋渡しする。
