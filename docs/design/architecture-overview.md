...
- 役割補足: `app/render.rs` は `run_ui_frame()`、`RenderCommand`、`dispatch_render_commands()`、小さな facade wrapper を保持する。results/status は `app/render_panels.rs`、top panel は `app/render_panels/top_panel.rs`、FileList/update/root-list dialog は `app/render_dialogs/` の private owner、tab/snapshot/theme は各 render module が担当し、interaction からの state transition は描画後 dispatcher で一段遅らせて実行する。render owner は filesystem/network/process/worker 実装へ依存しない。
