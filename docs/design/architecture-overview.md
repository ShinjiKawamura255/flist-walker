...
- 役割補足: preset picker は OS primary modifier の `Primary+Shift+P` から開く modal とし、catalog を専用 worker で非同期 load して名前を fuzzy filter する。Enter は pure search state だけを現在 tab へ適用し、Addは現在tabから新規draft、F2は選択中presetの編集draft、Deleteは確認状態へ遷移する。presetとNamed Rootのadd/replace/deleteは同じworker経由でatomic更新する。Named Root renameは参照presetを追従させ、deleteはsnapshot fallbackへ移行する。catalog編集は背景actionや現在tabを変更せず、メインpanelへpreset controlは常設しない。
...
- 実装: `rust/src/updater.rs`, `rust/src/updater/release.rs`, `rust/src/updater/staging.rs`, `rust/src/updater/manifest.rs`, `rust/src/updater/apply.rs`, `rust/src/updater/transaction.rs`, `rust/src/updater/transaction/model.rs`, `rust/src/updater/transaction/filesystem.rs`, `rust/src/updater/transaction/platform.rs`, `rust/src/main.rs`, `rust/src/app/update.rs`, `rust/src/app/render.rs`, `rust/src/app/workers.rs`, `rust/src/app/worker_tasks.rs`, `rust/src/app/state.rs`
