...
- Spelling advice and settled query errors render independently below the query; input-pending for an empty known field suppresses the redundant empty-field error. GUI contract tests drive pointer/key events through normal rendering into captured action/search channels, without intercepting the callback under test.
