...
- 各 request は data 8 batch と fixed Started/Truncated/terminal slot を持つ request-scoped mailbox を所有する。publication は mailbox-local sequence で merge し、terminal は final sequence と preceding-data count を固定する。producer は Full 時も cancel/close/shutdown を観測でき、coordinator は Active、Warm fixed quota、stale terminal の順で drain する。production では shared unbounded response channel と deferred non-active queue を使用しない。
