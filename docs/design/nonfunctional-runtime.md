...
- Walker metrics summary は `adaptive_limit_final` に加えて `adaptive_limit_avg`、`adaptive_limit_change_count`、`child_dir_publish_batches`、`max_queued_dirs`、`shared_frontier_capacity`、`frontier_saturation_fallbacks` を含む。これにより最後の値だけでなく、走行全体の平均並列度、変化回数、親列挙中の child frontier 公開回数、frontier peak、capacity、局所処理への切替回数を比較できる。`adaptive_limit_avg` は停止・join の尾を少し含みうる時間加重平均として扱う。
...
- 並列 worker は発見した child directory を 32 件単位で共有 FIFO frontier へ公開し、幅の広い親を最後まで列挙する間も idle worker が child を処理できるようにする。共有 frontier capacity は最大 worker 数 × 64 directory とする。32 件分の空きがない場合は mutex/condvar 上で待たず、親の `ReadDir` と未公開 child を明示的な frame stack に保持して pending child を同じ worker で局所処理し、完了後に親列挙を再開する。これにより共有 queue は固定上限、各 frame の未公開 child は 32 件未満となり、幅に比例する共有 frontier 成長と全 producer 待機を避ける。候補対象の親 directory entry は frontier 公開または局所処理より先に channel へ送信するため parent-before-child を維持する。global task の active count は局所 subtree 処理中も保持し、frame stack 全体の終了後だけ従来の mutex 境界で decrement して terminal 判定する。
