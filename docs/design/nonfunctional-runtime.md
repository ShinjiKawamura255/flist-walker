...
- adaptive traversal は GUI/TUI request の `include_files` / `include_dirs` を producer まで伝播する。単一種別モードでは通常 entry のうち対象外種別を path materialization と entry channel 送信の前に除外するが、対象外の通常ディレクトリも再帰 frontier には残す。symlink / Windows shortcut は target kind が未確定なら downstream classification へ残す。両種別モードは compile-time に filter 分岐を除去した worker / serial fast path を使う。
