...
- closed history は25件の lightweight intent を LIFO で保持し、元 index、sort/PIN/selection intent を含める。heavy snapshot は open-inactive と共通 LRU（count 2、weight 1,000,000 entry-unit）のみで保持する。active committed/building と sole Warm は pinned である。Evicted closed intent は新 tab id と generation で Loading し、旧 routing を継承しない。
- heavy snapshot は固定 reclaimer workerへ ownership transfer して UI thread 外で drop する。reclaimer queue は4件で、Full 時は payload を cache-owned eviction-pending として count/weight 内に残す。retirement を必要とする terminal commit は固定 terminal-pending slot で保留し、debt 解消まで同 tab の次 generation と新規 heavy admissionを止める。worker runtime は admission停止、generation無効化、mailbox close/wake、request endpoint切断、bounded cleanup、named index/reclaimer join、残endpoint closeの順でshutdownする。
