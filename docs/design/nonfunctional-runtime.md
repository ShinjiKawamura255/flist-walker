...
- Walker metrics summary は `adaptive_limit_final` に加えて `adaptive_limit_avg`、`adaptive_limit_change_count`、`child_dir_publish_batches`、`max_queued_dirs` を含む。これにより最後の値だけでなく、走行全体の平均並列度、変化回数、親列挙中の child frontier 公開回数、frontier peak を比較できる。`adaptive_limit_avg` は停止・join の尾を少し含みうる時間加重平均として扱う。
...
- 並列 worker は発見した child directory を 32 件単位で共有 FIFO frontier へ公開し、幅の広い親を最後まで列挙する間も idle worker が child を処理できるようにする。候補対象の親 directory entry を channel へ送信してから frontier へ公開するため、逐次表示では parent-before-child を維持する。残 batch は親の終了処理で公開し、active count と terminal 判定は従来どおり同じ mutex 境界で更新する。
