...
- sidecar を先に適用し、binary を最後の commit point とする。各操作直前に prepared new hash と既存 target old hash を marker と再照合する。Windows の既存 target は `[System.IO.File]::Replace(new, target, backup, false)`、Linux は create-new backup の file/parent sync 後に same-directory rename と parent sync を用いる。不在 target は destination を上書きしない same-directory hard-link promotion、parent sync、source unlink を用いる。
...
- copied helper の起動では installation directory を `current_dir` に設定せず、Windows の `CreateProcessW` が child current directory に課す legacy path length 制約から helper executable path を分離する。canonical helper path の起動が失敗し、path が verbatim drive/UNC 形式なら UTF-16 path identity を保った非 verbatim 表現で 1 回だけ再試行する。両試行の OS error は診断へ残し、利用者向け path は共有 display normalization と同じく verbatim prefix を除去する。
