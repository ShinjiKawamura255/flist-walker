...
- MUST: bounded lock wait、path canonicalize、latest-read merge、UI state / saved roots の atomic write は persistence worker が行い、frame dispatch を block してはならない。利用者が確定を要求する settings commit は request identity 付きの結果を返し、関連する複数ファイルの一方が失敗した場合は確定前の内容へ戻すか、rollback failure を明示する。opt-in window trace は bounded queue へ non-blocking enqueue し、満杯時は diagnostics event を破棄して UI frame を待たせない。graceful shutdown は frame rendering 外で bounded flush を要求する。crash-before-flush history loss と trace queue overflow による diagnostics loss は residual risk とする。
