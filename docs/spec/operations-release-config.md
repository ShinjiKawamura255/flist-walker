...
- MUST: tag release preflight は Linux/macOS/Windows native の各 job で `cargo test --locked` と `cargo clippy --locked --all-targets -- -D warnings` を実行し、OS 条件付き unused/dead code を release asset build 前に blocker として検出する。
...
- MUST: heavy PR CIはLinuxのclippy/coverage jobに加え、macOS/Windows native matrixで`cargo clippy --locked --all-targets -- -D warnings`を実行し、platform cfg固有warningをtag作成前にblockする。
...
### Regression Guard: native release warning gate

- Scenario: release preflightのclippyをLinuxだけに限定すると、macOS/Windowsの`cfg`でのみunused/dead codeになるwarningがrelease build logへ残ってもworkflowが成功する。
- Expected Behavior: heavy PR CIのmacOS/Windows native jobと、Linux/macOS/Windows nativeの全release preflight jobがlocked clippyを明示的な安全な`bash` shellで実行する。trusted policy testは対象job内のblocking stepを一意かつ許可されたプロパティ行の完全一致で要求し、PR側clippyの非Linux条件/command欠落、release側clippyへのplatform限定condition/command欠落、`continue-on-error`、`|| true`やcustom shell/envなどのfailure握り潰し、stepの別job移動または重複を拒否する。
- Non-goals: 通常PRのcoverage jobをplatformごとに複製すること、hosted runner queue時間の固定、warningのallowlist化。
- Related Tests: TC-198、`test_ci_contract_requires_non_linux_native_clippy_before_merge_regression`、`test_release_contract_requires_clippy_on_every_native_preflight_platform_regression`.
...
- MUST: 起動時 recovery は marker phase と旧新 hash から precommit rollback、完全な committed bundle、rolled-back bundle のいずれかへ収束させなければならない。live 登録 helper が存在する transaction と同時に回復してはならず、欠落 backup、hash 不一致、不正 state 遷移、path/type 変化は ambiguous として証跡を保持し、新しい update を開始してはならない。ambiguous state の利用者向け案内は [UPDATER_RECOVERY.md](../UPDATER_RECOVERY.md) へ誘導し、未検証 artifact の削除、改名、上書きを自動または一般手順として勧めてはならない。
- MUST: 通常のGUI/CLI起動時recoveryは最大5秒で`Deferred`を返して起動を長時間ブロックしてはならず、live helperのexecutable identityを確認できない場合は`Ambiguous`として証跡を保持しなければならない。一方、helperが起動する更新後processはrestart mode別の内部flagでcleanup handoff ownerであることを識別し、validated terminal markerのhelper PIDだけを対象にartifact無変更で最大30秒のbounded exit waitを行い、live中の一時的なidentity照会失敗でhandoffを中断してはならない。helper終了後はmarker identity/phaseを再検証し、通常のhash/classification recoveryでcommitted/rolled-back transactionをterminal cleanupへ収束させなければならない。GUI modeは回復完了後に内部flagを公開CLI引数へ渡さず通常GUIを続行し、Headless modeは回復後に終了する。30秒後も`Deferred`なら正常終了として扱ってはならない。この契約はUniversalと`fw`で共通とする。
- MUST: hidden helper の transaction/restart failure は、marker、transaction ID、helper PID/token/path/hash を検証済みの transaction context からだけ固定 basename の versioned diagnostic record として最大 16 KiB で永続化してよい。control character を sanitize し、利用者向け文字列の共有表示境界で文中の全 `\\?\` / `\\?\UNC\` prefix を除去し、create-new temporary file と atomic no-overwrite promotion を使い、symlink/reparse/wrong type を拒否し、診断記録失敗で rollback または元 error を置き換えてはならない。rollback 後に旧 binary を再起動する場合は、その process が起動診断を読むより前に record を publish しなければならない。信頼済み install directory を確定する前の failure は永続化対象外とする。
...
### Regression Guard: restart-cleanup-handoff

- Scenario: helperが新版を`binary_committed/applied`へ更新してGUIを生成した直後、新GUIの通常startup recoveryがlive helperの一時的なexecutable identity照会失敗を`Ambiguous`と判定し、更新済みversionであるにもかかわらず`Previous Update Failed`を表示する。またはHeadless childが`Deferred`のまま終了してcleanup ownerが消える。
- Expected Behavior: helperが起動するGUI/Headless processはmode別の内部restart flagを公開引数解析より前に処理し、validated terminal markerのhelper PID終了をartifact無変更で最大30秒待つ。終了観測後にmarkerとbundle hashを再検証してcleanupし、GUIだけ通常画面へ進む。更新診断とGUI noticeは共有文字列表示境界を通り、drive/UNC pathが文中に複数あっても`\\?\`を表示しない。
- Non-goals: 通常起動の5秒上限やfail-closed identity判定の緩和、live helperとの同時artifact変更、30秒を超えて停止したhelperの強制終了、filesystem APIへ渡すverbatim pathの変更。
- Related Tests: TC-159、TC-189、TC-191、TC-202、`tc159_headless_restart_handoff_outlives_normal_startup_deadline_regression`、`tc159_headless_handoff_waits_through_transient_helper_identity_failure_regression`、`tc202_regression_gui_restart_uses_internal_recovery_handoff`、`tc202_regression_failure_record_hides_embedded_windows_verbatim_prefixes`、`tc202_regression_display_text_strips_all_embedded_verbatim_path_prefixes`、`tc202_regression_status_notice_hides_embedded_verbatim_paths`。
- Notes for Future Changes: internal restart flag、process entry dispatch、startup recovery deadline、helper lifetime、failure/notice表示を変更するときはTC-202のfocused testとWindows GNU Universal/Fw E2Eを同一変更で実行する。
...
- MUST: GUI で `Use Ignore List` を切り替えた frame は committed candidate 全件を同期 scan/clone してはならない。変更は request identity 付きの bounded index refresh へ渡し、既存の表示 snapshot と現在の result sort mode/scope を維持したまま増分 filter と terminal settlement を行わなければならない。非 Score sort では、増分到着順の未sort結果を表示せず、同期sortまたは bounded sort/search worker の結果を適用できるまでsort済みlast-good snapshotを維持しなければならない。
...
- MUST: bounded lock wait、path canonicalize、latest-read merge、UI state / saved roots の atomic write は persistence worker が行い、frame dispatch を block してはならない。利用者が確定を要求する settings commit は request identity 付きの結果を返し、関連する複数ファイルの一方が失敗した場合は確定前の内容へ戻すか、rollback failure を明示する。replace 後の directory sync failure は該当 file を変更済みとして rollback し、元の durability error と rollback error の両方を保持しなければならない。opt-in window trace は bounded queue へ non-blocking enqueue し、満杯時は diagnostics event を破棄して UI frame を待たせない。graceful shutdown は frame rendering 外で bounded flush を要求する。crash-before-flush history loss と trace queue overflow による diagnostics loss は residual risk とする。
...
- MUST: GUI設定openは単一workerでpath解決・不足file生成・OS openを実行し、UI dispatchを待機させない。受理から完了まで重複要求を抑止して処理中を表示し、失敗後は再試行できる。不足 file 生成は sidecar lock 内の再確認後だけ行い、file生成には起動時の実効configを使い、設定を開く操作でprocess envへ再適用してはならない。
