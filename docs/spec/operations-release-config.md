...
- MUST: UI-state の保存前再読込は、ファイル未存在の場合だけ空 object を初期値とする。既存ファイルの read/UTF-8 failure、不正 JSON、object 以外の JSON は保存エラーとし、UI-state と関連する saved-roots の bytes を変更してはならない。通常の pending patch/history delta は保持し、ファイルが修復された後の再試行で最新 object へ merge する。明示 settings commit は request identity 付きエラーを返して再操作可能とする。起動時 reader の default fallback は既存ファイルの上書きを許可するものではない。
