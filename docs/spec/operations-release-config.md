...
- MUST: GUI で `Use Ignore List` を切り替えた frame は committed candidate 全件を同期 scan/clone してはならない。変更は request identity 付きの bounded index refresh へ渡し、既存の表示 snapshot を維持したまま増分 filter と terminal settlement を行わなければならない。
