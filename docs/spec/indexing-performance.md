...
- MUST: TC-156 は selective fuzzy、multi-token AND、exact、inverse/exclusion、anchor、OR、regex の sparse/dense shape と unknown-kind `ext:` shape を含め、cold/warm の path・score・order 一致と warm の評価候補削減を検証し、各 shape の maximum が 250ms 未満であることを PR/weekly regression ceiling として検証する。
...
- MUST: PR CI の Linux native release job と weekly perf workflow は TC-156 を明示実行する。
