...
- MUST: TUI index worker は選択 root の metadata error、missing、non-directory を `IndexFailed` として返し、空 index の正常完了へ変換してはならない。Walker source は GUI index worker と同じ adaptive traversal と runtime config の adaptive initial/max limit を使い、`walker_max_entries` 件で候補投入を停止して request identity 付き truncation 応答を terminal finish より先に返さなければならない。増分候補は immutable batch として共有し、入力ループ上の batch 追加で既存 path 全件を copy-on-write 複製してはならない。
