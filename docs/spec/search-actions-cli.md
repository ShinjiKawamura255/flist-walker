...
- MUST: presetの作成・削除はCLI/TUIとGUIで提供する。GUIはメイン画面へpreset controlを常設せず、OS primary modifierの`Primary+Shift+P`でpickerを開き、preset名のfuzzy filter、`Up` / `Down`選択、`Enter`適用、`Add`追加、`F2` / `Edit`編集、`Delete` buttonからの確認付き削除、`Esc` cancelを提供する。
