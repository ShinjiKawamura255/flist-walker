...
- Slice C: DONE on 2026-05-14. `app/input.rs` was converted into a facade plus interaction-family owner modules.
- Slice D: DONE on 2026-05-14. `indexer/mod.rs` and `search/mod.rs` were reduced to facade/implementation owners with tests moved under owner test modules.
- Slice E part 1: DONE on 2026-05-14. `app/filelist.rs` was converted into FileList owner modules.
- Slice E: DONE on 2026-05-14. `runtime_config.rs` and `app/session.rs` test bulk were moved under owner test modules after the FileList owner split.
- Slice F: DONE on 2026-05-14. `app/index_worker.rs` test bulk was moved under owner tests; `app/render_panels.rs` is below the first-wave threshold and was left unchanged.
- First implementation wave complete: no top-level Rust production file remains above 800 lines.

## Current Snapshot
2026-05-14 完遂時点の上位 production files:

| File | Lines | Primary concern |
| --- | ---: | --- |
| `rust/src/app/tabs.rs` | 745 | tab lifecycle, tab routing, drag/reorder, background responses |
| `rust/src/app/render_panels.rs` | 743 | top/status/results/preview panel rendering |
| `rust/src/app/session.rs` | 721 | UI state, saved roots/tabs, geometry, session persistence |
| `rust/src/app/pipeline.rs` | 703 | index/search orchestration and result application |
| `rust/src/app/index_worker.rs` | 642 | FileList stream, walker stream, kind classification, worker loop |
| `rust/src/app/worker_tasks.rs` | 552 | worker task entry points |
| `rust/src/runtime_config.rs` | 509 | config model, paths, env seed, migration |
| `rust/src/app/state.rs` | 508 | app state containers |
| `rust/src/updater/release.rs` | 477 | update release asset selection and version policy |
| `rust/src/app/shell_support.rs` | 456 | shell support paths and migration helpers |
| `rust/src/ui_model/highlight.rs` | 431 | search highlight and visible-match helpers |
| `rust/src/search/match_eval.rs` | 425 | query compilation and match evaluation |

2026-05-14 完遂時点の上位 app test modules:

| File | Lines | Notes |
| --- | ---: | --- |
| `rust/src/app/tests/render_tests.rs` | 557 | render regression owner |
| `rust/src/app/tests/index_pipeline/dialogs_and_inflight.rs` | 542 | dialog/inflight owner |
| `rust/src/app/tests/update_commands.rs` | 508 | update command owner |
| `rust/src/app/tests/session_restore.rs` | 506 | session restore owner |
| `rust/src/app/tests/index_pipeline/filelist_lifecycle.rs` | 499 | FileList lifecycle owner |
| `rust/src/app/tests/index_pipeline/kind_resolution.rs` | 407 | kind resolution owner |
| `rust/src/app/tests/query_history.rs` | 393 | query history owner |
| `rust/src/app/tests/window_ime.rs` | 369 | IME/window regression owner |
...
Completed moves:
- `app/input/mod.rs` is now the module facade.
- `actions.rs` owns selected paths, execute/open-folder, copy, clear pinned, and deferred copy handling.
- `navigation.rs` owns row/page movement, pin toggle, first/last row movement, and query/selection clearing.
- `text_editing.rs` owns query normalization, cursor/character helpers, Ctrl+H, and Emacs query editing shortcuts.
- `shortcuts.rs` owns top-level shortcut dispatch, root dropdown shortcut branch, history-search shortcut branch, tab shortcuts, and navigation/action shortcut routing.
- `dialogs.rs` owns FileList dialog input facade methods.
- `history.rs` owns query-history facade methods.
- `ime.rs` owns IME and space fallback event processing.
...
Completed moves:
- `indexer/mod.rs` now keeps the public facade and build-index implementation while `indexer/tests/mod.rs` owns contract tests.
- `indexer/tests/perf.rs` owns the ignored FileList stream perf guard while preserving the test name.
- `search/mod.rs` now keeps the public facade and search API implementation while `search/tests/mod.rs` owns search contract/ranking/regex/cache tests.
...
Completed FileList moves:
- `app/filelist/mod.rs` is now the facade.
- `commands.rs` owns the FileList reducer command surface and app dispatch.
- `manager.rs` owns FileList workflow request state, stale pending cleanup, cancellation state, and response context classification.
- `creation.rs` owns Create File List entry snapshots, overwrite/ancestor preflight, and reindex-before-create flow.
- `confirmations.rs` owns dialog confirmation/cancel helpers and in-flight Create File List cancellation.
- `responses.rs` owns FileList worker response routing and response side effects.

Completed remaining Slice E moves:
- `runtime_config/tests/mod.rs` owns runtime config migration, env seeding, path, and compatibility tests.
- `app/session/tests/mod.rs` owns session state path and legacy migration tests.
...
Completed moves:
- `app/index_worker.rs` now keeps worker implementation while `app/index_worker/tests/mod.rs` owns worker classification, trace, and perf tests.
- `app/render_panels.rs` was intentionally left unchanged in this wave because it is already below the 800-line success threshold; avoiding unnecessary GUI churn keeps this slice behavior-preserving.
