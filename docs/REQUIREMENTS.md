...
- 検索結果からフォルダをオープン。
- Python での CLI/GUI 試作実装とドキュメント化（SDD/TDD）。
...
- UC-004: 利用者が GUI 画面で候補を確認しながら、プレビューを見て対象を開く。
...
- FR-007: ツールは Python 試作で GUI（検索入力、結果リスト、プレビュー、実行/オープン）を提供し、UI 仕様を Rust 実装へ移植可能な形で文書化しなければならない。
...
- NFR-004: GUI 操作の主要フロー（検索、選択、アクション、再読込）は手動回帰手順を定義すること。
...
- AC-006: GUI で検索・選択・実行/オープン・再読込の基本操作が確認できる。
