...
- Large Rust file reduction first wave is complete; closure details are retained in this file.
...
- Notes:
  ...
  - 2026-05-14 に `large-rust-file-reduction` Slice C を完了した。`app/input.rs` を `app/input/mod.rs` facade、`actions.rs`、`navigation.rs`、`text_editing.rs`、`shortcuts.rs`、`dialogs.rs`、`history.rs`、`ime.rs` に分割し、既存の `FlistWalkerApp` input method names を維持した。最大 file は `input/shortcuts.rs` 298 行。`cargo test --locked shortcut`、`cargo test --locked window_ime`、`cargo test --locked`、`cargo clippy --all-targets -- -D warnings`、coverage gate `--fail-under-lines 75` は green。
  ...
  - 2026-04-25 に Slice C `updater-boundary-decomposition` を完了した。`updater.rs` の公開 API を維持したまま、release/candidate、staging/download、manifest verification、platform apply helper を private submodule へ分割し、private `VerifiedUpdateBundle` で apply spawn 前の署名/checksum 検証済み境界を固定した。`cargo test --locked`、`cargo clippy --all-targets -- -D warnings`、`cargo check --locked --target x86_64-pc-windows-gnu`、targeted updater/update command tests、`git diff --check` は green。
  - 2026-04-25 に Slice D `render-boundary-decomposition` を完了した。`render.rs` から stale duplicate panel/dialog/result-list drawing を削除し、`run_ui_frame()` / `RenderCommand` / facade wrapper / dispatcher に責務を限定した。描画 ownership は `render_panels.rs` / `render_dialogs.rs` / `render_tabs.rs` / `render_snapshot.rs` / `render_theme.rs` に集約し、`run_ui_frame` headless coverage を追加した。`cargo test --locked`、`cargo clippy --all-targets -- -D warnings`、render targeted tests、call-graph checks、touched-file `rustfmt --check`、`git diff --check` は green。
  - 2026-04-25 に Slice E `search-indexer-boundary-decomposition` の計画書を作成し、architecture/performance と testing/validation の specialist review を完了した。`search/mod.rs` は public facade / orchestration に寄せ、private query matching/evaluation は `search/match_eval.rs` へ切り出す。`indexer/mod.rs` は public facade / type/API owner に寄せ、nested FileList hierarchy override は `indexer/filelist_hierarchy.rs` へ切り出す behavior-preserving cleanup とする。pre-change search perf baseline は `search_100k_elapsed_ms=44`。
  - 2026-04-25 に Slice E を完了した。`search/match_eval.rs` が private query matching/evaluation を担当し、`indexer/filelist_hierarchy.rs` が nested FileList hierarchy override を担当する構造へ切り出した。public facade API は `search/mod.rs` / `indexer/mod.rs` に維持した。`cargo test --locked`、`cargo clippy --all-targets -- -D warnings`、coverage gate、search/indexer targeted tests、boundary checks、search 100k perf、VM-003 perf guards、`cargo audit`、`git diff --check` は green。`cargo audit` の既存 allowed transitive `paste` warning は Slice G へ継続 defer。
  - 2026-04-25 に Slice F `gui-validation-uplift` の計画書を作成した。GUI を CI で無理に起動するのではなく、deterministic fixture、manual smoke checklist、evidence path、owner/frequency、flake policy を持つ repeatable gate へ引き上げる。
  - 2026-04-26 に Slice F を完了した。`scripts/gui-smoke-fixture.sh`、`docs/GUI-TESTPLAN.md`、GUI report evidence path を追加し、`GSM-001` から `GSM-010` の GUI manual gate、ignored evidence path `rust/target/gui-smoke/`、owner/frequency/pass-fail/flake policy を定義した。`docs/TESTPLAN.md` の `TC-010` / `TC-011` / `TC-099` / `VM-002` / `VM-006` を新 gate へ接続した。original slice 時点では subagent quota exhaustion により独立 specialist review は未取得だったが、2026-04-26 の post-closure validation/security specialist review で blocker なしを確認した。
  ...
  - 2026-04-26 に post-closure GUI manual visual smoke を WSL2/WSLg 環境でユーザ確認し、`GSM-001` から `GSM-010` の PASS を記録した。Windows 11 / macOS の GUI smoke は release candidate 時の通常ゲートとして継続する。
  - 2026-04-26 に `quality-hardening-90` / `paste-audit-resolution` の完了済み `EXECUTION-PLAN` 群を削除した。恒久的な結果は `docs/TASKS.md`、`docs/OSS_COMPLIANCE.md`、`docs/GUI-TESTPLAN.md`、GUI report template、SDD 文書、コミット履歴に残す。
