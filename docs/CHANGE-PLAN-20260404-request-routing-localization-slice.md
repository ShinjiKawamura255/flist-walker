...
- Preview routing:
  ...
  - 対象責務は preview request 発行、preview response consume、preview routing cleanup の 3 点を同じ owner API に揃える
- Action/sort routing:
  - `tabs.rs` 側 owner へ寄せる
  - 理由: `close_tab_index()` による cleanup、tab snapshot (`pending_action_request_id`, `pending_sort_request_id`) の保存/復元、background tab への反映先解決が tab lifecycle に近接しているため
  - `mod.rs` には request 発行の入口と worker 応答の分岐だけを残し、request_id と tab_id の binding / consume / clear は `tabs.rs` owner API 経由に限定する
...
1. Phase 1: docs と routing ownership scaffolding
   ...
   - Gate: worker request/response の挙動変更を入れない。挙動変更が必要になった時点でこの Phase は VM-002 へ昇格し `cd rust && cargo test` を実行する。
...
3. Phase 3: action/sort routing owner localization
   - Files: `rust/src/app/mod.rs`, `rust/src/app/tabs.rs`, `rust/src/app/state.rs`, 必要なら `rust/src/app/tab_state.rs`
   - Action: action/sort routing を `tabs.rs` owner API へ寄せ、`mod.rs` から routing map の直接 read/write を除去する。
   - Verification: `cd rust && cargo test app_core -- --nocapture`; `cd rust && cargo test session_tabs -- --nocapture`
4. Phase 4: cleanup API 整理と regression 固定
   ...
   - Action: close cleanup と stale response regression を owner-localized 形に揃え、preview/action/sort の 3 系統を phase gate で固定する。
...
- preview は `cache.rs`、action/sort は `tabs.rs` owner としてコードと docs で説明可能になっている。
- `mod.rs` から preview/action/sort routing map の直接 read/write が除去されている。
