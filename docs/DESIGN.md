...
- `Drop` 時の worker `join` はタイムアウト付き（既定 2000ms）で待機し、超過時はタイムアウト件数を記録して UI 終了の無限待ちを回避する。
...
- 起動時の優先順位は `--root` 明示 > 復元タブ（env 有効時） > `Set as default` > 通常 root とし、常時復元で default root を無効化しない。
