...
- [app/render_panels.rs](../../rust/src/app/render_panels.rs) owns results/status and delegates top controls to `app/render_panels/top_panel.rs`; [app/render_dialogs.rs](../../rust/src/app/render_dialogs.rs) delegates FileList/update/root management, contextual shortcut help, and the contextual preset picker/editor to private owners under `app/render_dialogs/`. The help owner derives platform/runtime-dependent labels without I/O and keeps only ephemeral open state in `RuntimeUiState`. The preset owner keeps catalog/picker/editor draft state under `FeatureStateBundle`, loads and atomically replaces catalog entries through a worker, filters the accepted snapshot in memory, and applies through existing root/index/search/sort transitions. Modal input handlers prevent background command leakage; editor save does not invoke apply transitions. [app/render_tabs.rs](../../rust/src/app/render_tabs.rs), [app/render_snapshot.rs](../../rust/src/app/render_snapshot.rs), and [app/render_theme.rs](../../rust/src/app/render_theme.rs) own tabs, snapshots, and theme colors. Render owners mutate through existing owner methods or queued `RenderCommand` values and do not import filesystem/network/process/worker implementations.
