...
- Parallel adaptive workers publish discovered child directories to the shared FIFO frontier in batches of 32 before a wide parent finishes enumeration. An emitted parent directory is sent to the entry channel before its frontier publication, preserving parent-before-child progressive delivery. The terminal remainder and active-count decrement stay in the existing final mutex boundary. Summary metrics expose publish-batch count and peak queued directories; the serial path reports its local frontier peak and zero publish batches.
