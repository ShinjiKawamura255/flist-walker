...
- Parallel adaptive workers publish discovered child directories to the shared FIFO frontier in batches of 32 before a wide parent finishes enumeration. The frontier capacity is `max_workers * 64`. When fewer than 32 slots remain, a worker does not block on capacity: it suspends the parent `ReadDir` in an explicit frame stack and processes one pending child locally, then resumes the parent. Shared queue size is therefore fixed by capacity and each suspended frame retains fewer than 32 unpublished children, without recursive call-stack growth. An emitted parent directory is sent to the entry channel before frontier publication or local descent, preserving parent-before-child progressive delivery. The global task keeps its active slot across local subtree work and decrements it only after the frame stack drains. Summary metrics expose capacity, saturation fallback count, publish count, and peak queued directories; the serial path remains unchanged.
