...
| GSM-008 | Help, dialog, and error handling | Open shortcut help from both `Help` and `F1`; verify the OS primary modifier and current Emacs setting, then close it with `F1`, `Esc`, and `Close` while checking that background search/action state is unchanged. Also open a fixture-local confirmation/cancel path. Use only an injected/forced update-check failure; do not perform a network update check. | Help content matches the current platform/configuration; modal focus and close paths do not leak input to the background. Confirmation/error dialog focus, default action, cancel action, and returned notice are clear and do not leak stale state. |
...
| GSM-008 | `surface-dialog-theme`, `shortcut-help`, `stale-routing` | Help snapshot/content, modal command/input isolation, dialog commands, cancel/failure state, stale response rejection. | Native focus/default-button behavior and visible shortcut layout remain native. |
