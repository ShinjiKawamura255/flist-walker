...
| GSM-008 | Help, preset picker/editor, Named Root manager, dialog, and error handling | Open shortcut help from both `Help` and `F1`; verify the OS primary modifier, current Emacs setting, and preset/Named Root guidance, then close it with `F1`, `Esc`, and `Close` while checking that background search/action state is unchanged. Create preset fixtures through CLI, open the GUI picker with `Ctrl+Shift+P` or `Cmd+Shift+P`, fuzzy-filter by name, move selection, apply with Enter, and cancel with Esc. Open the selected preset with `F2` and `Edit`, edit every pure-search field, cancel once to verify no mutation, then rename and save with the primary modifier plus Enter and `Save`. From both the picker heading's `Manage named roots...` and the editor's `Manage...`, add an absolute root, rename/repath it, verify linked preset selection follows the rename, then exercise delete confirmation and verify the preset falls back to its snapshot. Verify nested Esc behavior, collision/relative-path/write errors retain the draft, catalog saves do not apply to the current tab, and the main panel has no preset control. Also open a fixture-local confirmation/cancel path. Use only an injected/forced update-check failure; do not perform a network update check. | Help, picker/editor, and manager content match the current platform/configuration; modal focus and close paths do not leak input to the background. Loading/error/empty/no-match/saving/deleting states are understandable, pure-search apply uses the selected preset, catalog mutation preserves unknown fields and reference/fallback contracts, and confirmation/error dialog focus, default action, cancel action, and returned notice are clear without stale state. |
