...
| GSM-008 | Help, preset picker, dialog, and error handling | Open shortcut help from both `Help` and `F1`; verify the OS primary modifier, current Emacs setting, and preset-picker guidance, then close it with `F1`, `Esc`, and `Close` while checking that background search/action state is unchanged. Create preset fixtures through CLI, open the GUI picker with `Ctrl+Shift+P` or `Cmd+Shift+P`, fuzzy-filter by name, move selection, apply with Enter, and cancel with Esc. Verify that apply changes search state but never opens/executes a result and that the main panel has no preset control. Also open a fixture-local confirmation/cancel path. Use only an injected/forced update-check failure; do not perform a network update check. | Help and picker content match the current platform/configuration; modal focus and close paths do not leak input to the background. Loading/error/empty/no-match states are understandable, pure-search apply uses the selected preset, and confirmation/error dialog focus, default action, cancel action, and returned notice are clear without stale state. |
...
| GSM-008 | `surface-dialog-theme`, `shortcut-help`, `preset-picker`, `stale-routing` | Help/picker snapshot and content, modal command/input isolation, pure-search-only apply, dialog commands, cancel/failure state, stale response rejection. | Native picker typing/focus, visible layout, and default-button behavior remain native. |
