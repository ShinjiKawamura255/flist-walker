...
| GSM-008 | Help, preset picker/editor, dialog, and error handling | Open shortcut help from both `Help` and `F1`; verify the OS primary modifier, current Emacs setting, and preset-picker/editor guidance, then close it with `F1`, `Esc`, and `Close` while checking that background search/action state is unchanged. Create preset fixtures through CLI, open the GUI picker with `Ctrl+Shift+P` or `Cmd+Shift+P`, fuzzy-filter by name, move selection, apply with Enter, and cancel with Esc. Open the selected preset with `F2` and `Edit`, edit every pure-search field, cancel once to verify no mutation, then rename and save with the primary modifier plus Enter and `Save`. Verify the renamed row is selected, saving does not apply it to the current tab, collision/write errors retain the draft, and the main panel has no preset control. Also open a fixture-local confirmation/cancel path. Use only an injected/forced update-check failure; do not perform a network update check. | Help and picker/editor content match the current platform/configuration; modal focus and close paths do not leak input to the background. Loading/error/empty/no-match/saving states are understandable, pure-search apply uses the selected preset, editor save mutates only the catalog while preserving unknown fields, and confirmation/error dialog focus, default action, cancel action, and returned notice are clear without stale state. |
...
| GSM-008 | `surface-dialog-theme`, `shortcut-help`, `preset-picker`, `stale-routing` | Help/picker/editor snapshots and content, modal command/input isolation, pure-search-only apply, atomic edit request/response, rename/collision/unknown-field handling, cancel/failure state, stale response rejection. | Native picker/editor typing/focus, combo layout, error readability, and default-button behavior remain native. |
