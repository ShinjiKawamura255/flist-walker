# EXECUTION PLAN: v0.18.9 Publish
...
- Scope Label: v0.18.9 publish
- Review Status: レビュー済み
- Review Viewpoints: security, rollback, operability, release-integrity

## Goal
- Immutable tag `v0.18.9` から全assetを生成し、検証済みreleaseを公開する。

## Execution Strategy
1. Remote `refs/tags/v0.18.9` が存在しないことを確認する。既存なら更新・削除せず停止する。Version/tag/changelog/HEADを再照合し、annotated tagを作成・pushする。準備commit SHAとlocal/remote peeled SHAの一致を確認し、tag object SHAとpeeled commit SHAを記録する。
2. `Release Tagged Build` を完了まで監視する。
3. Draft releaseから全assetを再downloadし、期待24 asset完全一致、22 checksum entry、`sha256sum -c`、署名公開鍵検証、全archive内の `LICENSE.txt` / `THIRD_PARTY_NOTICES.txt` 双方、standalone sidecarの双方、version、`.app` bundle非混入を確認する。
4. 固定formatでrelease本文を設定し、macOS未notarizedを明記する。
5. workflow log warningと本文policyを確認する。warningは原則publish blockerとし、例外時は本文、影響、承認者、承認理由をpre-publish reviewと恒久記録へ残す。
6. pre-publish specialist reviewで `docs/RELEASE_INCIDENT_RUNBOOK.md` の具体的な判断基準、GitHub Release/update feed操作、担当、証跡保存先も確認する。
7. Draftをpublishし、公開状態を確認する。
