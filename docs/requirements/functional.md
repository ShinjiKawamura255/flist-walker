...
- FR-006: ツールは CLI モード（`--cli`）で GUI を起動せず、batch 検索結果を機械処理可能な形式で出力し、明示的な root・sort・action・FileList 操作を安全に受理し、`--interactive` では検索・複数 pin・選択結果出力を行う端末 UI を提供しなければならない。interactive UI の描画は結果用 stdout と分離し、stdout を pipe した利用を妨げてはならない。
