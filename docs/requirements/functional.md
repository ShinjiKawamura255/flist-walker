...
- FR-006: ツールは CLI モード（`--cli`）で GUI を起動せず、batch 検索結果を機械処理可能な形式で出力し、明示的な root・sort・action・FileList 操作、および全結果を argv として渡す外部 command 実行を安全に受理しなければならない。外部 command は実行環境の引数上限まで結果をまとめ、結果0件では起動してはならない。`--color auto|always|never` は、既定の機械処理可能な出力を維持しつつ、明示した場合だけ ANSI 色出力を制御できなければならない。`--interactive` では検索・複数 pin・選択結果出力または端末復旧後の外部 command 実行を提供し、描画を結果用 stdout と分離しなければならない。
