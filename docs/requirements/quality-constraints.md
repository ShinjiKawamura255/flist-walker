...
- NFR-015: GUI index resource は active 1件と warm background 1件だけを pinned とし、その他の open-inactive/closed heavy snapshot を共通 LRU の件数・deterministic weight 上限で管理しなければならない。index response は request-scoped の bounded mailbox と順序付き control/terminal slot を使い、UI 側に無制限 queue を作ってはならない。heavy payload の最終 drop は固定 reclaimer worker が担当し、queue Full、cancel、failure、shutdown のいずれでも UI thread の同期 drop、terminal 消失、上限外の side queue を生じさせてはならない。これは cacheable history の上限であり、単一 active FileList snapshot の hard byte cap ではない。
