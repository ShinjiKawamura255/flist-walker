...
- NFR-011: interactive CLI の index/search は入力ループをブロックせず、同一 query を含む古い応答を request identity で破棄し、結果更新中も利用者の選択を可能な限り維持しなければならない。正常終了、cancel、setup 途中失敗、描画/入力エラー、unwind の各経路で terminal mode・alternate screen・cursor・bracketed paste を開始前相当へ復旧しなければならない。
