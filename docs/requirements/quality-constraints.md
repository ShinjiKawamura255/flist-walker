...
- NFR-009: GUI の通常・session restore・closed-tab restore のタブ遷移は、active tab の大規模な index、building queue、kind resolution、incremental result、search result を要素単位で複製・同期破棄・capacity 縮小・再構築せず所有権移動で遷移し、候補件数に比例する同期処理によって UI 入力応答性を損なってはならない。tab は lifecycle と committed snapshot の有無を別々に所有し、refresh 中または失敗後も last-good snapshot を表示できなければならない。直前に十分利用された open-inactive tab は短い往復操作で committed snapshot を再利用でき、偶発的な瞬間表示がその保護対象を置換してはならない。lazy load が必要な tab は同じ遷移処理内で `Indexing...` と最新 generation を確立し、Ready tab の activation は無条件 reindex してはならない。固定 10 万件の release-mode coordinator fixture ではタブ切替 p95 16ms を目標、50ms を hard ceiling とし、非同期 index 完了時間と resource reclamation はこの閾値から分離する。
...
- NFR-012: FileList transaction、history/UI-state/saved-root persistence、および opt-in window trace は UI frame / terminal event loop をブロックしてはならない。利用者が確定を要求する設定保存は成功または失敗を request identity とともに UI へ返し、失敗時に未保存値を確定済みとして表示してはならない。FileList の未 settle な worker は selection 出力・root 切替・terminal 終了より先に完了または失敗として確定しなければならない。
...
- NFR-014: Windows の固定 shallow 200-file fixture では、同一 release build・同一 host で 5 warmup 後に標準出力を破棄して 25 回測定した `fw` batch 検索の median が、universal の direct-process `flistwalker --cli` baseline median の 70% 以下でなければならない。この baseline は Windows universal の shell-synchronous CLI サポートを意味しない。1 回だけ全測定を再実行でき、再度未達なら release を停止する。
- NFR-015: GUI index resource は active 1件と warm background 1件を scheduler owner として pinned にし、それと直交する Recent Inactive 最大1件およびその他の open-inactive/closed heavy snapshot を、soft/hard の件数・deterministic weight 上限で管理しなければならない。Recent Inactive の猶予は通常の cache pressure だけを延期し、hard pressure は猶予中でも回収できなければならない。index response は request-scoped の bounded mailbox と順序付き control/terminal slot を使い、UI 側に無制限 queue を作ってはならない。heavy payload の最終 drop は固定 reclaimer worker が担当し、queue Full、cancel、failure、shutdown のいずれでも UI thread の同期 drop、terminal 消失、上限外の side queue を生じさせてはならない。これは cacheable history の上限であり、単一 active FileList snapshot の hard byte cap ではない。
