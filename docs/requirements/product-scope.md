...
- AC-038: CLI は positional QUERY を予約語化せず、明示 long option で named root と検索 preset を list/add/save/remove/applyでき、`--preset` は batch/TUI の検索開始時に適用できる。GUI は `Ctrl+Shift+P`（macOS は `Cmd+Shift+P`）で modal picker を開き、名前を fuzzy filter して `Enter` で現在 tab の pure search state へ適用し、`Esc` で現在の検索状態を変えず閉じる。GUI のメイン画面には preset control を追加せず、catalog の作成・編集・削除は CLI/TUI に限定する。旧 `.flistwalker_roots.txt` と保存済み catalog は変更されず、preset 適用だけでは副作用 action を開始しない。
