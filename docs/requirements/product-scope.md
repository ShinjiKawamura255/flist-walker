...
- AC-038: CLI は positional QUERY を予約語化せず、明示 long option で named root と検索 preset を list/add/save/remove/applyでき、`--preset` は batch/TUI の検索開始時に適用できる。GUI は `Ctrl+Shift+P`（macOS は `Cmd+Shift+P`）で modal picker を開き、名前を fuzzy filter して `Enter` で現在 tab の pure search state へ適用し、`Esc` で現在の検索状態を変えず閉じる。`Add` は現在tabのpure-search stateから新規draftを作り、選択中presetは`F2` / `Edit`から編集、`Delete` buttonから確認付き削除できる。追加・編集・削除はatomic catalog updateだけを行い現在tabへ適用しない。picker と preset editor から開く Named Root 管理画面は absolute path の追加、名称・path編集、確認付き削除を提供し、名称変更は参照presetを追従、削除は保存済みpath snapshotへフォールバックさせる。GUI のメイン画面には preset control を追加しない。旧 `.flistwalker_roots.txt` と保存済みunknown fieldは保持され、preset適用・catalog編集だけでは副作用actionを開始しない。
