...
- AC-038: CLI は positional QUERY を予約語化せず、明示 long option で named root と検索 preset を list/add/save/remove/applyできる。GUI は current root の命名と current search の保存、preset の選択・適用・削除を提供し、catalog 書込み中も frame をブロックしない。旧 `.flistwalker_roots.txt` は変更されず、preset 適用だけでは副作用 action を開始しない。
