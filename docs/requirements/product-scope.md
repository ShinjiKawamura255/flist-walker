...
- AC-038: CLI は positional QUERY を予約語化せず、明示 long option で named root と検索 preset を list/add/save/remove/applyでき、`--preset` は batch/TUI の検索開始時に適用できる。GUI は named-root/preset の管理・適用 control を表示せず、既存の root list、tab、query history を利用する。旧 `.flistwalker_roots.txt` と保存済み catalog は変更されず、preset 適用だけでは副作用 action を開始しない。
