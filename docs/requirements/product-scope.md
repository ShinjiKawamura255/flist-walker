...
- AC-041: `fw QUERY --root PATH` は `flistwalker --cli QUERY --root PATH` と同じ stdout/stderr/exit code を返し、`fw` の help/program name は短い command 名を表示する。release には既存 archive を変更せず各 platform/arch の `fw` standalone asset が追加され、Windows の固定 200-file fixture で NFR-014 の起動性能基準を満たす。
