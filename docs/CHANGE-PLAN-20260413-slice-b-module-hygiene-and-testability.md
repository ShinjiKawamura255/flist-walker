...
- Risk: splitting modules without introducing clear seams would produce more files without reducing coupling.
  ...
  - Mitigation: pair file splits with explicit imports, command collection seams, and targeted tests.
...
1. Split the densest UI/control modules into smaller seams.
   ...
   - Success condition: the largest modules are smaller and the control flow is easier to follow.
...
3. Address the remaining data-flow / observability rough edges.
   ...
   - Improve error/notice handling so the remaining observability story is less dependent on a single notice string path.
   - Success condition: the remaining performance and operational rough spots are narrowed enough to support closure.
