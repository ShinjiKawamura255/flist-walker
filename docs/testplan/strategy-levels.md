...
- routing / cleanup の確認は `rust/src/app/response_flow.rs`、`rust/src/app/result_reducer.rs`、`rust/src/app/index_coordinator.rs`、`rust/src/app/pipeline.rs`、`rust/src/app/tab_state.rs`、`rust/src/app/worker/bus.rs` を owner seam として扱い、background response の stale discard は `tab_background_responses.rs`、tab close cleanup は `tab_lifecycle.rs` / `tab_result_cache.rs`、index lifecycle cleanup は `index_pipeline/filelist_lifecycle.rs` へ寄せる。
...
- Coverage: CI の `lint-and-coverage` job は `cargo llvm-cov --locked --workspace --lcov --output-path target/llvm-cov/lcov.info --fail-under-lines 75` を実行し、line coverage 75% 未満への低下を失敗扱いにする。ローカルでは先に `target/llvm-cov` を作成する。2026-07-30 の final closure baseline は 78.10%（LH=21280 / LF=27246）で、TUIのインラインテストをowner別ファイルへ移し worker lifecycle owner を明示した後のproduction-oriented計測を反映する。中期目標は80%とする。enforced threshold を上げる変更では、同一変更内で fresh baseline、失敗時の不足領域、追加した owner-seam test を記録する。
