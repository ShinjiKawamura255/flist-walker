...
- 期待動作: startup precedence、lifecycle/committed分離、Active+sole Warm、順序付きbounded mailbox、live/closed共通LRU、engagement-qualified Recent Inactive、soft/hard budget、bounded reclaimerをTC-203からTC-211で同時に確認する。Readyと猶予中のRecent Inactiveは再indexせず、瞬間表示だけのtabは保護枠を奪わず、Refreshing/Failedはlast-good、Dormant/EvictedだけLoadingする。
- 非対象範囲: full snapshotのprocess間永続化、単一active FileList snapshotのhard byte cap。
- 関連テストID: TC-203, TC-204, TC-205, TC-206, TC-207, TC-208, TC-209, TC-210, TC-211.
...
- Automated evidence: `cargo test cli_tui::tests --lib` は stdin/stderr TTY policy、stdout非依存、partial setup、runtime error/unwind時の逆順復旧、cleanup-before-output、request freshness、選択/pin、editor/paste/page移動、Unicode幅と制御文字の安全なclipに加え、invalid root の `IndexFailed`、candidate batch の path allocation 共有、iteration ごとの response budget を検証する。`cargo test --test cli_contract` は batch 出力・段階別 progress・interactive/batch option 契約を検証する。
...
- Windows ConPTY parity follow-up (2026-07-26): 現行native debug binaryを120x30で起動し、preview本文、`Alt+P` hide/show、`F1` helpとcurrent-row action案内、78x20から120x32へのresize、bracketed paste、Esc cancel、Enter選択を確認した。stdoutはraw file handleへredirectし、cancel時は空、選択時は相対path 1行のみ、Escはexit 130、全sessionでalternate screen退出、cursor再表示、bracketed paste無効化sequenceを確認した。結果: `conpty_preview=ok conpty_help=ok conpty_resize=ok conpty_paste=ok conpty_cancel=130 conpty_selection=ok restoration=ok stdout_redirect=ok`。実OS action launchは追加の明示承認を伴わないため実行せず、key dispatch、current-row限定、事前認可、backend call 0件の拒否経路はTC-164のrecording backendで検証した。
- Windows ConPTY operations follow-up (2026-07-26): 隔離したcurrent/saved rootとsettingsを使い、`F2`でFiles toggleを適用・復元、`F3`でName (A-Z) sort、`F4`でsaved root切替、`F5`で切替後rootの再index、`F6`でroot-only/ancestor scope表示と`Ctrl+G` cancelを実操作した。FileListは作成されず、最後のEscはexit 130、stdoutは空、terminal restoration sequenceも確認した。結果: `conpty_f2_toggle=ok conpty_f3_sort=ok conpty_f4_root_switch=ok conpty_f5_refresh=ok conpty_f6_confirmation_cancel=ok no_filelist_write=ok conpty_cancel=130 restoration=ok stdout_redirect=ok`。
...
- TC-120 -> SP-010, SP-014, SP-016 -> DES-015, DES-017, DES-014 -> FR-007, FR-019, NFR-012
...
- TC-163 -> SP-006, SP-013 -> DES-005, DES-006 -> FR-006
- TC-164 -> SP-004, SP-006 -> DES-004, DES-005 -> FR-006, FR-009
- TC-165 -> SP-001, SP-006 -> DES-005, DES-007 -> FR-010
- TC-166 -> SP-001, SP-006 -> DES-005, DES-007 -> FR-006, FR-010, NFR-012
- TC-167 -> SP-010, SP-016 -> DES-009, DES-017 -> FR-007, FR-011, FR-026, NFR-012
- TC-168 -> SP-016 -> DES-015, DES-017 -> FR-011, FR-026, NFR-012
...
- TC-201 -> SP-006, SP-010 -> DES-005, DES-009 -> FR-041, AC-042
- TC-202 -> SP-014 -> DES-014 -> FR-033, NFR-010
- TC-215 -> SP-006, SP-014, SP-018, SP-023 -> DES-005, DES-014, DES-019, DES-024 -> FR-032, FR-033, FR-040, NFR-010, NFR-014, AC-033, AC-036
