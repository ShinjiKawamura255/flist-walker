...
- Verify required workflows use numbered runner generations, Rust/tool versions, full Action SHAs, least permissions, timeout/concurrency, image-version evidence, and download-only Cargo caches; `pull_request_target` is forbidden.
...
- Review `CI Gate` aggregation, scheduled audit/canary issue tracking, Dependabot auto-merge registration, and the pin promotion/rollback rules in `docs/CI_OPERATIONS.md`.
- Repository setting changes require before/after API read-back and a protected machine PR that auto-merges without admin bypass.
...
| VM-009 CI reliability / pins / merge policy | `.github/workflows/*`, `.github/dependabot.yml`, `rust/rust-toolchain.toml`, `rust/.cargo/audit.toml`, `scripts/check_ci_policy.py`, `scripts/tests/test_check_ci_policy.py`, `docs/CI_OPERATIONS.md`, repository branch protection/auto-merge settings | `python -m unittest scripts.tests.test_check_ci_policy`; PyYAML parse of all workflow files; `cd rust && cargo fmt --check`, `cargo clippy --locked --all-targets -- -D warnings`, `cargo test --locked`, `cargo audit`; TC-056B audit skip negative/safe-skip contract review; PR `CI Gate` | scheduled audit/canary manual dispatch、failure issue create/update、required check source、approval 0、force/delete禁止、per-PR auto-merge と non-bypass merge を API/PR evidence で確認する |
