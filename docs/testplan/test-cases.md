...
| TC-174 | unit+integration+headless+manual | catalog name validation、unknown-field round trip、newer/malformed read-only failure、sidecar-lock RMW、legacy roots 不変、named-root snapshot fallbackを検証する。CLI は management/apply と positional `root`/`preset`/`catalog` query 互換、TUI startup preset を検証する。GUI は picker open/filter/select/apply/cancelに加え、`F2` / `Edit` draft生成、全pure-search field編集、rename/collision、absolute root検証、unknown-field保持、`Primary+Enter` / `Save`成功・失敗、`Esc` draft破棄、最新response、editor保存時の現在tab不変と副作用action 0件、F1 help、メイン画面にpreset controlがないことをunit/integration/headless snapshotとGSM-008で確認する | SP-019 |
