...
| TC-174 | unit+integration+headless | catalog name validation、unknown-field round trip、newer/malformed read-only failure、sidecar-lock RMW、legacy roots 不変、named-root snapshot fallbackを検証する。CLI は management/apply と positional `root`/`preset`/`catalog` query 互換、TUI startup preset を検証する。GUI surface snapshot は named-root/preset の状態と action を公開しないことを確認する | SP-019 |
