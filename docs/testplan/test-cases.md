...
| TC-158 | unit+platform | `VerifiedUpdateBundle` の activation 準備は executable parent 内の create-new `.new`/lock/marker/backup に限定し、directory/symlink/reparse/parent 外/衝突を拒否する。sidecar を先、binary を最後の commit point とし、Windows existing target は `File.Replace(..., false)`、Linux existing target は同期済み backup + same-directory rename、不在 target は no-overwrite move/rename を使う | SP-014 |
| TC-159 | unit+integration | deterministic failure injection は per-target write-ahead state、parent/helper registration と acknowledgement の全 crash point、ack 前 no-mutation、live helper recovery exclusion、30秒 parent wait、各 sidecar/binary 操作、restart failure、startup recovery、ambiguous-state retention、cleanup ownershipを検証する。precommit/restart failure は旧 bundle、committed state は完全な新 bundleへ hash 検証付きで収束する | SP-014 |
| TC-160 | platform | Windows PowerShell と Linux shell の同一 filesystem 上にある inert dummy transaction は成功、precommit failure rollback、restart failure rollback、中断 recovery、atomic primitive、cleanup を実証する。実行中 FlistWalker binary、production update feed、外部 application を置換・起動しない | SP-014 |
