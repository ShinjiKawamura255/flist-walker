...
| TC-056E | operations | `master` はPR、approval 0、GitHub Actions由来の`CI Gate`と`CI Policy Guardian`、force/delete禁止、linear history、rebase-only、merge済みbranch自動削除を要求する。2commit以上のmachine PRがadmin bypassなしでrebase auto-mergeされ、境界・順序・message・author・patch/tree対応を保ち、追加commitが各1 parentでmerge commitを含まないことを確認する | SP-012 |
