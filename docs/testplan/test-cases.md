...
| TC-169 | unit+integration | TUI update notice は英語の `Update available` と終了後の `flistwalker --update` 手動実行案内を含み、check result の受信だけでは apply を開始しない。`--check-update` / `--update` は排他的 CLI 操作として help に英語で掲載され、更新確認無効時、up-to-date、candidate、manual-only、check/apply failure の出力と exit code を検証する | SP-006, SP-014 |
