...
| TC-179 | unit+platform+regression | updater helper command は installation directory を child current directory に固定しない。Windows verbatim drive/UNC helper path の初回 spawn 失敗は UTF-16 identity を保った非 verbatim path で 1 回だけ再試行し、通常 path は再試行しない。全試行失敗の診断は OS error を保持しつつ `\\?\` prefix を表示しない | SP-014 |
