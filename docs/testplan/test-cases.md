...
| TC-174 | unit+integration+GUI | catalog name validation、unknown-field round trip、newer/malformed read-only failure、sidecar-lock RMW、legacy roots 不変、named-root snapshot fallbackを検証する。CLI は management/apply と positional `root`/`preset`/`catalog` query 互換、TUI startup preset を検証する。GUI は background persistence と typed apply transition を owner test および VM-002 smoke で確認する | SP-019 |
