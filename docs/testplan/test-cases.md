...
| TC-173 | unit+headless+manual | GUI の `Help` と `F1` はショートカットモーダルを開き、`F1` / `Esc` / `Close` で閉じる。表示内容は OS primary modifier と `emacs_keybindings_enabled` を反映し、表示中のコピー等の背景ショートカットは状態を変更しない。headless snapshot と GSM-008 で表示契約を確認する | SP-010 |
