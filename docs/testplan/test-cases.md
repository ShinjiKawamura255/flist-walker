...
| TC-156 | perf | 計測区間外で構築した固定 10万件 fixture を release mode で selective fuzzy、multi-token AND、exact、inverse/exclusion、anchor、OR、regex の sparse/dense shape により5回以上測定し、compile、cold、warm、評価候補数、match 数、median、maximum を報告する。代表 cold/warm median は 100ms 未満を目標とし、weekly CI の全 shape は各 sample と maximum が 250ms 未満、warm は cold と同じ path/score/orderで評価候補数を削減する | SP-007 |
