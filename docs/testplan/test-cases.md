...
| TC-172 | unit+integration | CLI は `--color auto|always|never` を受理し、batch の `auto` は TTY のみで色を出力して pipe/redirect の path-only stdout framing を維持する。`NO_COLOR`、`always`、`never` と interactive CLI の画面描画への適用を検証する | SP-006 |
