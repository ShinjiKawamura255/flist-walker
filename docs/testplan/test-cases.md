...
| TC-152 | unit | index executor は stale を含め固定 2 worker + queue 2（総量 4）、coordinator tracking は最大 2、app pending は全体最大 4 かつ tab ごとに最新 1 件を維持する。`Full` は latest-only pending へ戻し、stale/cancel は canonicalize/FileList/walker I/O より前に `Canceled` response、`Disconnected` は terminal failure として routing/in-progress を解放する | SP-010 |
...
| TC-191 | integration+platform+CI | test-channel公開鍵を埋めたWindows GNU artifactをWindows runnerへ渡し、実行可能なまま明確にhashが異なるoverlay付きupdate payloadを作成し、全取得先をloopback HTTP、署名をtest keyだけに限定したheadless sandbox更新を実行する。更新前後で異なるbinary hash、sidecar、marker/lock消滅、restart child終了をtimeout付きで確認し、server traversal、sandbox子孫のreparse cleanup、signing key子process継承、job削除/skip/test keyのproduction混入をpolicy negative testで拒否してCI Gateへ集約する | SP-014 |
