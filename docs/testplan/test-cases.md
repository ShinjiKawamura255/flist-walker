...
| TC-163 | integration | batch CLI は `--sort` の全 mode、sort-before-limit、`limit=0`、root selector mutual exclusion、saved-root one-based listing、legacy print framing を検証する。`--list-saved-roots` は index/action/write 0 回、invalid root selector は indexing 前 exit 2 とする | SP-006, SP-013 |
