...
| TC-056E | operations | `master` はPR、approval 0、GitHub Actions由来の`CI Gate`と`CI Policy Guardian`、force/delete禁止、linear history、rebase-only、merge済みremote branch自動削除を要求する。auto-merge 実完了後、clean worktree の active task だけが `master` を fast-forward 同期し、マージ済みかつ未使用の local feature branch だけを通常削除することを確認する。dirty worktree、未マージ PR、fast-forward 不能、または他 worktree が使用する branch では後処理が停止する。2commit以上のmachine PRがadmin bypassなしでrebase auto-mergeされ、境界・順序・message・author・patch/tree対応を保ち、追加commitが各1 parentでmerge commitを含まないことを確認する | SP-012 |
