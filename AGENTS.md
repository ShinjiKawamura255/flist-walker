...
- PR 作成 agent は `skills/flistwalker-pr-lifecycle/` に従い、`gh pr merge --auto --rebase --delete-branch` 相当を1回だけ明示登録する。auto-merge が実際に完了したことを確認した active task は、作業ツリーが clean な場合に限り、`origin/master` を fetch してローカル `master` を fast-forward 同期し、作業 branch から `master` へ戻る。マージ済み local feature branch は、他 worktree で使用中でなく `master` に到達可能な場合だけ削除する。未マージ、同期失敗、dirty worktree、または branch 使用中では停止して状態を報告し、強制 reset / 強制 branch 削除はしない。remote feature branch は GitHub の auto-delete に委ねる。ローカルでは意味のある単位で自由にコミットして feature branch へ push してよい。必要な履歴整理の force-with-lease は非保護 feature branch に限り許可し、`master` の force push、branch deletion、admin bypass で gate を回避しない。Dependabot PR は trusted `workflow_run` から同じ auto-merge を登録する。
