...
- PR 作成 agent は `gh pr merge --auto --rebase --delete-branch` 相当を1回だけ明示登録する。ローカルでは意味のある単位で自由にコミットして feature branch へ push してよい。必要な履歴整理の force-with-lease は非保護 feature branch に限り許可し、`master` の force push、branch deletion、admin bypass で gate を回避しない。merge 後の remote feature branch は自動削除する。Dependabot PR は trusted `workflow_run` から同じ auto-merge を登録する。
