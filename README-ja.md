...
- Windows では実行ファイル横、Linux/macOS では `~/.flistwalker_config.json` による runtime config / session files
...
- 永続化先は Windows では実行ファイル横、Linux/macOS ではホーム直下の平文ファイルです。検索語やパスに機微情報を含める運用では注意してください。
...
- runtime settings は Windows では実行ファイル横の config file、Linux/macOS では `~/.flistwalker_config.json` と関連する session file に集約され、同名 env は seed 用です。
