...
flistwalker_require_windows_gnu_tools() {
  ...
  local linker cxx ar ranlib windres
  ...
  if [[ -z "${linker}" || -z "${cxx}" || -z "${ar}" || -z "${ranlib}" || -z "${windres}" ]]; then
    return 1
  fi
...
個別指定する場合は FLISTWALKER_WINDOWS_LINKER / _CXX / _AR / _RANLIB / _WINDRES を設定してください。
EOF
}
