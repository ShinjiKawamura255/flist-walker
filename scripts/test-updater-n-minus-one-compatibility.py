...
def test_regression_v0251_accepts_exact_v0260_inventory(
    script: Path, manifest: Path, digest: str
) -> None:
    write_manifest(manifest, release_inventory("0.26.0"), digest)

    result = run(script, manifest, "0.25.1", "0.26.0")

    assert result.returncode == 0, result.stderr


def test_regression_v0260_accepts_exact_v0270_inventory(
    script: Path, manifest: Path, digest: str
) -> None:
    write_manifest(manifest, release_inventory("0.27.0"), digest)

    result = run(script, manifest, "0.26.0", "0.27.0")

    assert result.returncode == 0, result.stderr


def test_regression_v0270_accepts_exact_v0271_inventory(
    script: Path, manifest: Path, digest: str
) -> None:
    write_manifest(manifest, release_inventory("0.27.1"), digest)

    result = run(script, manifest, "0.27.0", "0.27.1")

    assert result.returncode == 0, result.stderr


def test_regression_v0271_accepts_exact_v0280_inventory(
    script: Path, manifest: Path, digest: str
) -> None:
    write_manifest(manifest, release_inventory("0.28.0"), digest)

    result = run(script, manifest, "0.27.1", "0.28.0")

    assert result.returncode == 0, result.stderr


def test_regression_v0280_accepts_exact_v0290_inventory(
    script: Path, manifest: Path, digest: str
) -> None:
    names = release_inventory("0.29.0")
    assert len(names) == 26
    write_manifest(manifest, names, digest)

    result = run(script, manifest, "0.28.0", "0.29.0")
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory() as temp:
        ...
        test_regression_v0251_accepts_exact_v0260_inventory(script, manifest, digest)
        test_regression_v0260_accepts_exact_v0270_inventory(script, manifest, digest)
        test_regression_v0270_accepts_exact_v0271_inventory(script, manifest, digest)
