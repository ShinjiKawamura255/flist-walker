...
impl FlistWalkerApp {
    pub(in crate::app) fn gui_help_lines(emacs_keybindings_enabled: bool) -> Vec<String> {
        ...
        let mut lines = vec![
            ...
            "Add / Delete — Save the current pure-search state or remove the selected preset after confirmation"
                .to_string(),
            ...
            "Applying a preset updates search state only; it never opens or executes a result"
                .to_string(),
            "Adding, editing, or deleting updates the catalog only; it does not apply a preset"
                .to_string(),
            "".to_string(),
