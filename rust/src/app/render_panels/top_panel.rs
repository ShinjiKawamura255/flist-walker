    ...
    egui::Panel::top("top").show(ui, |ui| {
        ...
        ui.horizontal_wrapped(|ui| {
            ...
            let preset_names = app
                ...
                .collect::<Vec<_>>();
            egui::ComboBox::from_id_salt("search_preset_selector")
                .selected_text(selected_label)
                .show_ui(ui, |ui| {
                    for name in preset_names {
                        ui.selectable_value(
                            &mut app.shell.features.presets.selected_name,
                            Some(name.clone()),
                            name,
                        );
                    }
            ...
            if ui
                .add_enabled(
                    !busy && app.shell.features.presets.selected_name.is_some(),
            ...
            if ui
                ...
                .clicked()
            {
                app.request_remove_selected_preset();
            }
            ui.add(
                egui::TextEdit::singleline(&mut app.shell.features.presets.name_input)
                    .desired_width(110.0)
                    .hint_text("Preset name"),
            );
            if ui
                .add_enabled(!busy, egui::Button::new("Save current"))
                .clicked()
            {
                app.request_save_current_preset();
            }
            ui.add(
                egui::TextEdit::singleline(&mut app.shell.features.presets.root_name_input)
                    .desired_width(100.0)
                    .hint_text("Root name"),
            );
            if ui
                .add_enabled(!busy, egui::Button::new("Name root"))
                .clicked()
            {
                app.request_name_current_root();
            }
            if !app.shell.features.presets.error.is_empty() {
                ui.colored_label(
                    ui.visuals().error_fg_color,
                    app.shell.features.presets.error.clone(),
                );
            }
        });
