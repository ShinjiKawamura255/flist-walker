...
7. `cargo clippy --all-targets -- -D warnings` を実行し、Rust warning / clippy warning が残っていないことを確認する。
8. release asset build または GitHub Actions の release build logs に warning が出ていないことを確認する。warning が残る場合は publish 前に修正するか、release blocker ではない理由と follow-up を明記する。
9. tag 名 `vX.Y.Z`、`CHANGELOG.md`、`rust/Cargo.toml`、`rust/Cargo.lock` の version が一致していることを確認する。
10. この確認が終わるまで tag 作成・push・draft release publish を行わない。
11. release 本文が必要なら `skills/flistwalker-release-notes/SKILL.md` を続けて使う。
